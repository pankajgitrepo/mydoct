...
namespace mojoPortal.Web.UI
{
    public partial class CommentsWidget : UserControl, IRefreshAfterPostback, IUpdateCommentStats
    {
        ...
        //End
        private string mblog = string.Empty;
        public string MBlog
        {
            get { return mblog; }
            set { mblog = value; }
        ...
        {
            //ParentGuid Extracted from Querystring
            if (Request.QueryString != null && Request.QueryString.Count > 0)
            {
                if (Request.QueryString["parentguid"] != null)
                    ParentGuid = new Guid(Request.QueryString["parentguid"].ToString());
                else if (Request.QueryString["mediaId"] != null)
                {
                    string mediaId = (string)Request.QueryString["mediaId"];
                    ParentGuid = GetGuidforMedia(mediaId);
                }
                else 
                {
                    MBlog = "blog";
                ...
                Session["ParentGuid"] = ParentGuid;
            }

            //ParentGuid = Request.QueryString["parentguid"] != null ? new Guid(Request.QueryString["parentguid"].ToString()) : Guid.NewGuid();
            //Session["ParentGuid"] = ParentGuid;
            //End
        ...
        /// <summary>
        /// Custom Method Added for Kaltura. Gets Guid based on media id
        /// </summary>
        /// <param name="mediaId"></param>
        /// <returns></returns>
        private Guid GetGuidforMedia(string mediaId)
        {
            return (new KalturaModule()).GetGuidforMedia(mediaId);
        }
        ...
        private void BindComments()
        {
            ...
            if (Session["ParentGuid"] != null)
            {
                if (string.IsNullOrEmpty(MBlog))
                {
                    ParentGuid = new Guid(Session["ParentGuid"].ToString());
                    rptComments.DataSource = GetCommentsByParent();
                }
                else
                {
        ...
        private List<Comment> GetCommentsByParent()
        {
            if (sortDescending)
            {
                return repository.GetByParentDesc(ParentGuid);
            }
            else
            {
                return repository.GetByParentAsc(ParentGuid);
            }
        }
        ...
        protected string GetCommentDuration(DateTime startDate)
        {
            string duration = string.Empty;
            if ((int)(DateTime.Now - startDate).TotalDays > 365)
                duration = (((int)(DateTime.Now - startDate).TotalDays) / 365).ToString() + " year(s) ago";
            else if ((int)(DateTime.Now - startDate).TotalDays < 365 && (int)(DateTime.Now - startDate).TotalDays > 30)
                duration = (((int)(DateTime.Now - startDate).TotalDays) / 30).ToString() + " month(s) ago";
            else if ((int)(DateTime.Now - startDate).TotalDays < 30 && (int)(DateTime.Now - startDate).TotalHours > 24)
                duration = ((int)(DateTime.Now - startDate).TotalDays).ToString() + " day(s) ago";
            else if ((int)(DateTime.Now - startDate).TotalHours < 24 && (int)(DateTime.Now - startDate).TotalMinutes > 60)
                duration = ((int)(DateTime.Now - startDate).TotalHours).ToString() + " hour(s) ago";
            else if ((int)(DateTime.Now - startDate).TotalMinutes < 60 && (int)(DateTime.Now - startDate).TotalSeconds > 60)
                duration = ((int)(DateTime.Now - startDate).TotalMinutes).ToString() + " minut(s) ago";
            else if ((int)(DateTime.Now - startDate).TotalSeconds < 60)
