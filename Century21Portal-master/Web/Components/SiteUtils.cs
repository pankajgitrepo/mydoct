...
namespace mojoPortal.Web
{
    ...
    public static class SiteUtils
    {
        ...
        {
            string redirectUrl 
                = string.Format(CultureInfo.InvariantCulture, GetLoginRelativeUrl() + "?returnurl={0}",
                              HttpUtility.UrlEncode(HttpContext.Current.Request.RawUrl));
       ...
       // only place that calls this is CSSValidatorLink.cs
        ...
        public static SiteUser CreateUserFromQueryString(SiteSettings siteSettings, SsoQueryString ssoQueryString)
        {
            if (siteSettings == null)
            {
                throw new ArgumentException("a valid siteSettings object is required for this method");
            }
            if (string.IsNullOrEmpty(ssoQueryString.UserId))
            {
                throw new ArgumentException("a valid email address is required for this method");
            }

            if (!Email.IsValidEmailAddressSyntax(ssoQueryString.UserId))
            {
                throw new ArgumentException("a valid email address is required for this method");
            }
            ...
            {
                Email = ssoQueryString.UserId,
                Occupation = ssoQueryString.OfficeName,
                Country = ssoQueryString.Country,
                State = ssoQueryString.State,
                Name = ssoQueryString.FullName,
                FirstName = ssoQueryString.FirstName,
                LastName = ssoQueryString.FullName.Replace(ssoQueryString.FirstName,"").Trim(),
                City = ssoQueryString.City,
                Address1 = ssoQueryString.Address1,
                Address2 = ssoQueryString.Address2,
                Zip = ssoQueryString.Zip,
                AccountId =  ssoQueryString.AccountId,
                ProfileApproved = true,
                LoginName = ssoQueryString.FirstName
            };

            siteUser.Save();
            string queryStringRole = ssoQueryString.Access;
            if (ssoQueryString.Access.Equals(Role.AdminRole))
            {
                queryStringRole = Role.AdministratorsRole;
            }

            if (!siteUser.IsInRoles(queryStringRole))
            {
                Role r = Role.GetRoleByName(siteSettings.SiteId, queryStringRole);
                if (r != null)
                {
                    Role.AddUser(r.RoleId, siteUser.UserId, r.RoleGuid, siteUser.UserGuid);
                }
                else
                {
                    /* this requirement suggested by Abhinandan on 29/07/2013 */
                    //If role not present into database then assign unknown role to user
                    r = Role.GetRoleByName(siteSettings.SiteId, Role.UnknownRole);
                    if (r != null)
                    {
                        Role.AddUser(r.RoleId, siteUser.UserId, r.RoleGuid, siteUser.UserGuid);
                    }
            ...
            if (!siteUser.IsInRoles(Role.AuthenticatedUserRole))
            {
                Role.AddUserToDefaultRoles(siteUser);
            }
        ...
        public static bool IsFileExist(string fullFilePath)
        {
