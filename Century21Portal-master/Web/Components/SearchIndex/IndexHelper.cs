                ...
                try
                {
                    ...
                    { 
                        ...
                        for (int i = startHit; i < itemsToAdd; i++)
                        {
                            ...
                            #region Get Kaltura Video Thumbnail URL from  01/09/2015
                            //Contents has the Content + OtherContents refer to Century21Portal\Web\Components\SearchIndex\KalturaIndexBuilderProvider.cs
                            string cont = doc.Get("contents");
                            int index = cont.LastIndexOf("http://");
                            //Get the thumbnail url into OtherContent
                            if (index != -1 && indexItem.ModuleId == Convert.ToInt32(ConfigurationManager.AppSettings["KalturaPlayVideosModuleID"]))
                            {
                                indexItem.OtherContent = cont.Substring(index);
                            }
                            #endregion
