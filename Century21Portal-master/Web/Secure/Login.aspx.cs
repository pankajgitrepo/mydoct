...
namespace mojoPortal.Web.UI.Pages
{
    public partial class LoginPage : NonCmsBasePage
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            //Custom code for getting query string parameters
            //var queryStringParamsCount = Request.QueryString.Count;
            //if (queryStringParamsCount > 0)
            //{
            //    string requestURL = Request.Url.OriginalString;
            //    //Uri requestUri = new Uri("http://www.yoursite.com?t=2010:01:01:12:30&w=12345&a=98765&x=Joe.Smith&o=45454&c=0001&f=Joe&z=c1beee00921da2210032b&sid=ZF165AFD-¬‐0134-¬‐451A-¬‐8986-¬‐2ABE972539DZ");
            //    Uri requestUri = new Uri(requestURL);
            //    string[] queryParams = GetQueryStringData(requestUri.OriginalString);
            //    if (queryParams != null)
            //    {
            //        //string personKey = queryParams[1];
            //    }
            //}
            ...
            ////Custom code for getting query string parameters
            //var queryStringParamsCount1 = Request.QueryString.Count;
            //if (queryStringParamsCount1 > 0)
            //{
            //    string requestURL1 = Request.Url.OriginalString;
            //    //Uri requestUri = new Uri("http://www.yoursite.com?t=2010:01:01:12:30&w=12345&a=98765&x=Joe.Smith&o=45454&c=0001&f=Joe&z=c1beee00921da2210032b&sid=ZF165AFD-¬‐0134-¬‐451A-¬‐8986-¬‐2ABE972539DZ");
            //    Uri requestUri = new Uri(requestURL1);
            //    string[] queryParams = GetQueryStringData(requestUri.OriginalString);
            //    if (queryParams != null)
            //    {
            //        //string personKey = queryParams[1];
        ...
        //Custom function
        private string[] GetQueryStringData(string input)
        {
            int index = input.IndexOf('?');
            if (index != -1)
            {
                var query = input.Substring(index + 1);
                var queryParameters = input.Substring(index + 1).Split('&');
                WriteToNotepad(queryParameters);
        ...
        //Custom function
        private void WriteToNotepad(string[] queryParameters)
        {
            string startupPath = System.IO.Directory.GetCurrentDirectory();
            string strPath = Server.MapPath("~");
            StreamWriter singleLineWrite = new StreamWriter(strPath + @"\\QueryStringTextFile\\QueryString.txt", true);
            singleLineWrite.Write(Environment.NewLine);
            singleLineWrite.Write("----------------------" + "New record saved at: " + DateTime.Now.ToString() + "----------------------" + Environment.NewLine);
            singleLineWrite.Close();
            System.IO.File.AppendAllLines(strPath + @"\\QueryStringTextFile\\QueryString.txt", queryParameters);
        }
